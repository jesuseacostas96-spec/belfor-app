...
def extract_equipment(messages):
    import anthropic
    # FIX 1: Updated model name to valid current model
    ...
    for i in range(0, len(messages), 50):
        ...
        try:
            resp = client.messages.create(
                model="claude-sonnet-4-20250514",  # FIX 1: was "claude-sonnet-4-5" (invalid)
                max_tokens=4000,
                system=SYSTEM,
...
def build_excel(equipment, job_name, job_number):
    ...
    all_floors = sorted(
        set(list(floors_placed.keys()) + list(floors_removed.keys())),
        reverse=True
    )
    ...
    for i, floor_num in enumerate(all_floors):
        ...
        for idx, eq in enumerate(all_rows[:50]):
            row    = 13 + idx
            is_am  = eq["desc"] == "AM"
            is_rem = eq["action"] == "removed"

            status    = 6 if is_rem else 2
            bg        = REM_BG if is_rem else (AM_BG if is_am else DH_BG)
            txt_color = "C0392B" if is_rem else ("1A4F7A" if is_am else "7B5200")
    ...
    hset(ws_sum["A2"],
         f"{job_name or ''} | {job_number or ''} | Date(s): {', '.join(all_dates)}",
         BLUE, WHITE, sz=10, bold=False)
    ...
    loc_headers = ["#","Unit / Location","Type","AMs Placed","DHs Placed",
                   "AMs Removed","DHs Removed","Net AMs","Net DHs"]
    for ci, h in enumerate(loc_headers, 1):
        c  = ws_sum.cell(row=5, column=ci)
    ...
    for e in equipment:
        ...
        if key not in loc_summary:
            loc_summary[key] = {
                "unit": key,
                "type": "Hallway" if "hallway" in key.lower() else "Apartment",
    ...
    hset(ws_sum.cell(row,1),
         f"TOTALS — {apt_count} Apts | {hall_count} Hallways | {len(sorted_locs)} Locations",
         NAVY, GOLD, sz=10)
    tb(ws_sum.cell(row,1))
    for ci, val in [(4,tot_amp),(5,tot_dhp),(6,tot_amr),(7,tot_dhr),
                    (8,tot_amp-tot_amr),(9,tot_dhp-tot_dhr)]:
        bg = AM_H if ci in (4,6,8) else DH_H
        hset(ws_sum.cell(row,ci), val, bg, GOLD, sz=11)
        tb(ws_sum.cell(row,ci))
    ...
    for label, action, typ, bg, color in metrics:
        ...
        for di, date in enumerate(all_dates):
            col = di + 2
            if action == "net":
                # FIX 2: Use correct "action" field values (not default "placed") for both placed & removed
        ...
        c.font = Font(name="Arial", bold=True, size=11, color=GOLD if "Net" in label else color)
...
st.markdown("---")
st.markdown(
    "<div style='text-align:center;color:#999;font-size:0.75rem'>BELFOR Property Restoration • Powered by Claude AI</div>",
